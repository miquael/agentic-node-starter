...
> **Critical**: `.human/` must be added to `.gitignore`. AI agents must never access it.
...
- Ensure root `AGENTS.md` and `CLAUDE.md` are pointer files to `.ai/AGENTS.md`.
...
- [ ] Root `AGENTS.md` and `CLAUDE.md` point to `.ai/AGENTS.md`
- [ ] `.human/` exists with its README, is in `.gitignore`
